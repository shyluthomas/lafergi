...
class Packages_model extends MY_Model
{
	...
	function get_all($arr_where=NULL)
	{
		if($arr_where==NULL)
		{
			 $arr_result = $this->db -> order_by('order', 'asc')
						 			-> get($this->_table)
						 			-> result_array();
	...
	function get_all_search($get_all_search)
	{
		 $arr_result = $this->db -> like(array('catagory_type'=>urldecode($get_all_search)))
		 						 -> order_by('order', 'asc')
		 					 	 -> get($this->_table)
								 -> result_array();
	...
	function get_row_withuser($id)
	{
		...
		$arr_result = $this->db -> join('tbl_user', 'tbl_aboutus.user_id = tbl_user.user_id')
