...
class Products extends CI_Controller {
	  ...
	  public function index($test_id=NULL)
	 {
				...
				$arr_data_old=$this->products->get_all();
		 ...
		 $this->data['arr_view']=$this->products->get_all();		 
		 
		 $this->load->view('admin/products/manage_products',$this->data);
	 }
		 ...
		 if($this->input->post('save'))
		 {
			$array_form = $_POST;			
						...
						// unable to create this this thumbnail
							...
							$config['source_image'] = './assets/backend/images/products/'.$upload_data['file_name'];
							$config['create_thumb'] = FALSE;
							$config['maintain_ratio'] = false;
							$config['width']     = 270;
							$config['height']   = 358;
							$config['new_image'] =  './assets/backend/images/products/thump/'.$upload_data['file_name'];
			...
			$insert=$this->products->insert_data($array_insert);
			
			
		 }
		 
		 //$this->data['arr_view']=$this->productsus->get_all();		 
		 
		 $this->load->view('admin/products/add_products',$this->data);
	 }
	 ...
	 public function status($test_id,$status)
	 {
		 ...
		 $this->products->update_data($test_id,$arr_data);
	 ...
	 {
		 $this->data['arr_view']=$this->products->get_row_withuser($aboutid);
		 ...
		 $this->load->view('admin/products/view_products',$this->data);
	 ...
	 {
		 $this->data['arr_view']=$this->products->get_row_detail($aboutid);
		 ...
		 if($this->input->post('save'))
		 {
			...
			$executed = $this->products->update_data($aboutid,$array_insert);
			if($executed)
			{
				$this->session->set_flashdata('success', 'products page details updated successfully');
				redirect('/backend/products/');
			}
			else
			{
				$this->session->set_flashdata('fail', 'products page details updated unsuccessfully');
				redirect('/backend/products/');
			}
		  ...
		  $this->load->view('admin/products/edit_products',$this->data);
